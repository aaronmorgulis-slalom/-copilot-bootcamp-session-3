...
describe('Tasks API', () => {
  ...
  it('should create a new task with explicit priority', async () => {
    const res = await request(app)
      .post('/api/tasks')
      .send({ title: 'High Priority Task', description: 'Important', due_date: '2025-09-30', priority: 'P1' });
    expect(res.status).toBe(201);
    expect(res.body).toHaveProperty('id');
    expect(res.body.title).toBe('High Priority Task');
